...
async function load(){
 try{
  const r=await fetch('/api/account'),data=await r.json();if(r.status===401){location.href='account.html';return}if(!r.ok)throw Error(data.error||'Unable to load your orders.');
